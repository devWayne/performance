目前阿里大部分团队也还在用onload、webViewDidFinishLoad、performance.timing等类似时刻来衡量页面性能，这些指标都存在2个问题，1是无法衡量一个页面是否真的加载完了，2是它只能衡量一个单一的时刻，这个时刻促发得早，不一定给人的感觉就是快，例如加载时间1s，但前9.9s页面完全空白。
